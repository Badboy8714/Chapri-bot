...
module.exports = {
  ...
  onStart: async function ({ message, args, event, threadsData, role }) {
    const threadID = Number(event.threadID); // Convert to number
    if (!threadID || isNaN(threadID)) {
      return await message.reply("Invalid thread ID.");
    }
    ...
    if (args.length === 0) {
      ...
      msg += `━━━ 𝗦𝗨𝗣𝗣𝗢𝗥𝗧 𝗕𝗢𝗫 ━━━\n`;
      msg += `Join KHAN-Bot Zone ❈ Support Box type: ${prefix}supportgc or type: ${prefix}callad to contact with admins.\n\n`;
      ...
      if (searchResults.length > 0) {
        ...
        msg += searchResults.sort().map(cmd => `▸ ${cmd}`).join("\n");
      } else {
